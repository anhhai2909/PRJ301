...
public class addsubjectservlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(!d.checkexistedsubject(suid, suname, descript).isEmpty()){
        ...
        }else if(!"10".equals(grade.substring(0, 2))&&!"11".equals(grade.substring(0, 2))&&!"12".equals(grade.substring(0, 2))){   
            request.setAttribute("error", "**Thông tin cung cấp không hợp lệ**");
            request.getRequestDispatcher("addsubject.jsp").forward(request, response);
        }else{
            d.insertsubject(new subject(suid, suname, descript),grade);
